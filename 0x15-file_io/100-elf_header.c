...
int main(int argc, char *argv[])
{
	...
	file_directory = open(argv[1], O_RDONLY);
	if (file_directory == -1)
	...
	if (read(file_directory, header, sizeof(Elf64_Ehdr)) != sizeof(Elf64_Ehdr))
	...
	/* close file */
	if (close(file_directory) == -1)
		dprintf(STDERR_FILENO, "Error: Can't close file_directory %d\n", file_directory), exit(98);
